...
> d'opt-in`.
> **Les 2 constats corrigés**, vérifiés sur le banc (upgrade sur
> `2f2386ab`) :
>
...
(résolu 6/6).
En branchant la purge des sessions (F35), on constate que la
...
- **Note** : le banc était sur `5dbb382`.
  Depuis, `main` est passé à `bded0bb`,
...
- **Cause** : le handler CLI de l'opt-in (`forge_mvc_sessions_db/cli/gc.py`)
  ...
  du projet** (`env/dev`).
  Les commandes du cœur qui touchent la base
  (`forge run`, `forge migration:apply`) amorcent, elles, l'environnement.
  À noter
  ...
  (sinon `env/dev` n'est pas trouvé).
  La commande d'opt-in ne fait ni l'un ni l'autre.

- **Conséquence** : `sessions:gc` est **inutilisable en cron/systemd**, or c'est
  précisément l'usage prévu (purge périodique, réponse à F35).
  Le porteur doit
...
- **Contournement projet** : `tools/sessions-gc.py` : se replace dans la racine
  ...
  Robuste au CWD → fonctionne en cron/systemd.
...
- **Déjà corrigé pour ces commandes** : `bded0bb` (CLI-HELP-COVERAGE-OPTIN-INIT-001)
  ...
  couverture d'aide (`cli/_support/help_dispatch.py`).
  Le garde-fou central de
  `forge.py` (`wants_help(...)` → `format_command_help(command)` **avant**
  `dispatch_optin`) affiche alors l'aide et `return`.
...
- **Résidu (le vrai fond)** : ce garde-fou est **basé sur un enregistrement
  ...
  commande non listée, et le garde-fou ne fait alors rien.
  Or `dispatch_optin`
  ...
  **s'exécute encore** sur `--help`.
  La couverture est un rattrapage au cas par
...
- **Suggestion** : ajouter un **filet générique dans `dispatch_optin`** :
  ...
  aide de repli (docstring du handler / message neutre).
  Ainsi tout opt-in est
...
(durci et validé en retour-016).
Non bloquants pour le projet grâce au

...
`relations.sql`).
Le répertoire `mvc/models/sql/`, **où `auth:init` écrit**, est
**ignoré**.
Lancer `db:apply` comme recommandé ne crée donc **aucune** table auth
...
une commande dédiée (`forge auth:apply` ?).
À défaut, corriger le message de
...
Or la table `roles` appartient à l'**opt-in rbac** (non installé).
Appliquer le
socle **complet** échoue (FK vers table absente).
De même, `auth_mfa_*` relèvent
...
(différés).
Appliquée par `migration:apply`, tables créées, `auth:status` vert
sur le socle de base.

## Référence

Migration : `mvc/migrations/20260707163553_create_auth_socle.sql`.
Entité/flux :
ticket 07.
Voir aussi [retour-003](retour-003-forge-migrations-provisioning-manuel.md)

...
| [017](retour-017-sessions-gc-namorce-pas-la-config-et-help-sexecute.md) | `forge sessions:gc` n'amorce pas la config projet (`env/dev`) → `Access denied`, inutilisable en cron (F39) ; `--help` s'exécute au lieu d'afficher l'aide sur `sessions:gc`/`sessions:init` (F40) | À remonter |

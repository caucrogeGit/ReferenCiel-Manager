...
liste renvoie 500. Ces deux bugs sont **invisibles aux portes statiques** — seul un
...
`make:crud`), **non détectables** par les portes statiques. Ils confirment la valeur
d'un banc d'essai qui **exécute réellement** le parcours. Corrigés à la source,
...
`mvc/views/components/button.html`. Preuves : `core/auth/user.py:39`,
`cli/entities/crud/views_builder.py`. Flux : ticket 07 (commit `f29002c`).

...
la main. Trois défauts se cumulent.
...
- `make:relation` n'écrit **que** dans `mvc/entities/relations.json`
  (`cli/entities/make_relation.py:4`). Il n'ajoute pas de champ FK au contrat de
...
depuis `relations.json.foreign_key`). La contrainte vise donc une colonne inexistante.
...
`Id BIGINT UNSIGNED`. MariaDB **refuse** une FK entre types différents
...
n'est créée. `make check` reste vert (les JSON sont valides) — le blocage n'apparaît
...
mais le CRUD généré (depuis `classe.json`) ne gérerait toujours pas les FK. **Classe
est mise en pause** en attendant un correctif du flux relation.

## Référence

`cli/entities/relations.py:198`, `cli/entities/make_relation.py`. Preuves : `classe.sql`
(`AnneeScolaireId BIGINT`), `relations.sql` (`FOREIGN KEY (annee_scolaire_id)`),
`annee_scolaire.sql` (`Id BIGINT UNSIGNED`). Flux : ticket 07 (Bloc A, entité Classe).

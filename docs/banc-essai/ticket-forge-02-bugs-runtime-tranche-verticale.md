...
**vert**, et pourtant l'application **plante à l'usage**.
Ces bugs ne sont visibles
qu'en **exécutant** le parcours.
Ils échappent à `make:crud`/`make:auth` parce que
...
`retour-008`.
Preuves : `core/auth/user.py:39`, `cli/entities/crud/views_builder.py`,
`components/ui.html`.

...
- **Symptôme** : `make:crud <E>` lit `<e>.json` (champs propres) mais **pas**
  `relations.json`.
  Le contrôleur, le modèle et le formulaire générés **ignorent**
...
s'arrête là : ni la migration ni le CRUD ne « voient » les relations.
Pour une entité
liée, il faut compléter la migration à la main et le CRUD reste inutilisable en
création.
Combler F22/F23 rendrait la chaîne `make:entity → make:relation →

...
- **Défauts** :
  1. **Type incompatible** : `groupe_id` / `eleve_id` sont `INT`, mais les PK visées
     (`groupe.Id`, `eleve.Id`) sont **`BIGINT UNSIGNED`**.
     MariaDB **refuse** la FK
  ...
  3. **Casse de la PK référencée** : `REFERENCES groupe (id)` (minuscule) alors que la
     colonne est `Id`.
     Sans effet sur MariaDB (noms de colonnes insensibles à la casse),
...
(colonnes `BIGINT UNSIGNED`, `ENGINE` ajouté, `REFERENCES … (Id)`).
La table pivot
...
(`relations.py` / génération `sync:relations`).
Preuve : `mvc/entities/relations.sql`
(table `groupe_eleve`, colonnes `INT`).
Contournement : migration `create_groupe`.

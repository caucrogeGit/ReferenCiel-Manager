...
**colonne**, `sync:relations` la **contrainte + index**. Deux défauts se glissent
...
- **Cause** (`forge_mvc_entities/entity_semantic_validate.py`, règle 7, ~l.211-228) :
  ...
  "foreign_key"`**. Or ce champ **est** la FK que la relation déclare : ce n'est pas un
  « champ métier » en collision. La règle vise à détecter un vrai conflit (une colonne
...
- **Incohérence interne** : `make:relation` **produit** exactement cet état (champ
  ...
  le **rejette**. Un même Forge crée un état que son propre validateur refuse.

- **Impact** : `entity:validate` est **inutilisable** dès qu'une entité a une FK via le
  nouveau modèle (toute entité liée). Non bloquant pour `make check` (`project:check`
...
Le nouveau modèle FK (retour-011 résolu) fonctionne et produit un CRUD FK-aware. Restent
deux défauts en marge : une **porte qualité qui rejette l'état normal** (F26) et un
**découpeur SQL** trompé par une apostrophe en commentaire (F27). Aucun ne bloque
...
`forge_mvc_entities/db_apply.py` (`split_sql_statements`). Preuves : sorties littérales
ci-dessus. Flux : Bloc A, entité `InscriptionEleve` (migration `create_inscription_eleve`).

...
Le dépôt public de Forge bouge souvent. Une application-banc d'essai doit **suivre
cette évolution régulièrement**. Or `forge new` sait **créer** un projet, mais il
n'existe **aucun chemin outillé pour mettre à jour le squelette d'un projet
existant**. Chaque application doit donc réinventer une procédure manuelle — et
...
1. **Le framework** (`forge-mvc`, pin git) : mise à jour = bump du pin +
   ...
   `pip install --force-reinstall --no-deps`. Un projet naïf croit avoir mis à
...
2. **Le squelette** (`app.py`, `mvc/`, `optins/registry.py`, CI, gabarit ADR…) :
   ...
   au code utilisateur. Rien n'aide à distinguer les deux.

...
(`make:crud` → `router.group("/annee_scolaire")`).
On veut une vue prof derrière
...
- **Symptôme** : **aucune route protégée n'est atteignable**.
Un utilisateur non
  authentifié est redirigé (302) vers `/login`, qui **n'existe pas** → 404.
  Impossible
...
1. **Scaffolder optionnel** : `forge make:auth` (ou `forge auth:scaffold`) générant
   ...
   *write-if-new*, dans le style de l'app (comme `make:crud`).
   Idéalement aussi
   `/logout`.
...
`core/security/decorators.py`, `core/security/middleware.py`.
Flux : ticket 07,

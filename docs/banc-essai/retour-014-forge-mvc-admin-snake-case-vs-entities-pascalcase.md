...
- **Conséquence** :
  ...
  - Les colonnes **multi-mots** sont **inutilisables** : `importe_le` ≠ `ImporteLe` →
    `Unknown column 'importe_le'`. Impossible d'afficher `ImporteLe`, `RefCode`,
...
- **Impact** : une entité `forge-mvc-entities` **ne peut pas** être exposée proprement en
  ressource `forge-mvc-admin`. Les deux opt-ins, pensés pour coexister, ne partagent pas
...
(`identifiant`, `version`, `statut`), `pk="id"` ; `ImporteLe` **absent** de la liste. La
...
(`SELECT <list_fields>`). Colonnes générées : `mvc/entities/*/*.sql` (PascalCase). Contournement :
`mvc/routes/admin_routes.py`. Flux : ticket 11 (parcours des référentiels importés).

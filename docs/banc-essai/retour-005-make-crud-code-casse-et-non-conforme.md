...
avec `forge make:crud`. Le code produit **ne s'importe même pas**, et échoue les
...
et importé de là. Un générateur ne devrait pas émettre un import qu'il ne satisfait pas.
...
pour qu'il passe les portes que Forge s'impose. Corriger à la source rendrait
`make:crud` utilisable *out of the box*. Voir aussi [retour-004](retour-004-code-genere-non-conforme-portes-qualite.md)
(même nature côté `make:entity`).

## Référence

Contournements : `mvc/helpers/flash.py`, `mvc/models/annee_scolaire_model.py`
(typé). Entité de test : `AnneeScolaire` (ticket 07).

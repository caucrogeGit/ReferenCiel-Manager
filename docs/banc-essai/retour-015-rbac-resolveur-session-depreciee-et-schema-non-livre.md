...
- Contexte : discriminer la **navigation** et **protéger les routes** par rôle
  (admin gère le socle scolaire ; professeur conçoit/évalue/suit).
  Contrat
...
- **Cause** : `forge_mvc_rbac/contract.py::get_request_roles` résout les rôles
  ...
  Or l'auth moderne (`core.auth.session.login_user`)
...
- **Conséquence** : l'opt-in RBAC, tel quel, **n'est pas câblable** sur l'auth
  moderne.
  Les deux briques officielles de Forge (auth moderne + RBAC) ne se
  parlent pas.
  Aucun message n'alerte : on croit à un problème de contrat ou
...
- **Suivi (2026-07-12)** : correction en **deux temps** côté Forge.
  ...
  - **RÉSOLU sur `e7cc3f1a`** : `get_request_roles` **n'appelle plus** `get_user`.
    Il
    ...
    repli sur la session legacy.
    Le RBAC contractuel natif est donc **câblable** sur
  ...
  - **Complété côté Forge** (mêmes patchs) : résolveur `get_request_roles`
    ...
    **garde par préfixe** (`PrefixPermissionMiddleware`, `5ef21fa3`).
    Les trois
  ...
  - **Conséquence projet, RÉALISÉE** : la couche maison RBAC (`mvc/services/rbac.py`)
    ...
    vérifiée e2e par rôle (home connectée comprise).
    Plus aucun contournement RBAC.
...
- **Symptôme** : le resolveur DB (`forge_mvc_rbac/resolver.py`) interroge
  **quatre tables** :
  ...
  → vide).
  Il faut écrire **à la main** tout le schéma (4 tables) et le seed.

- **Aggravant** : le contrat `mvc/security/rbac.json` (rôles → permissions) et
  ces tables sont **deux mondes disjoints**.
  La CLI de l'opt-in n'expose que
  `rbac:validate` et `rbac:audit` : **pas de `rbac:sync`** pour projeter le
  contrat dans `permissions`/`role_permissions`.
  Le resolveur DB ne verra donc
...
- **Conséquence** : deux sources de vérité à tenir manuellement synchronisées,
  sans outil.
  Cf. déjà signalé partiellement en retour-006 (socle auth/RBAC non
...
1. **Rôles depuis la session moderne + base** : `get_authenticated_user_id`
   (moderne) → jointure `user_roles`/`roles`.
   C'est le maillon que F30 rate.
...
- **F30** : brancher `get_request_roles` sur l'auth moderne : résoudre `user.id`
  ...
  session dépréciée.
  À défaut, documenter explicitement l'incompatibilité et le
  pont à écrire.

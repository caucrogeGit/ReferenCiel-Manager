...
profit du natif.
Ce document reste comme trace du *pourquoi* du contournement.
**Date :** 2026-07-10
...
- **F30** : son resolveur (`get_request_roles`) lit `core.security.session`
  (**module déprécié**, dict utilisateur avec champ `roles`).
  Or l'auth moderne
  (`core.auth.session.login_user`) ne stocke que `user.id`.
  → rôles **toujours
...
1. **Rôles = session moderne + base.** `get_authenticated_user_id` (auth moderne)
   → jointure `user_roles`/`roles`.
   Pas de dépendance à la session dépréciée.
2. **Décision = contrat.** On retient le modèle **contractuel** (`rbac.json`),
   via `has_contract_permission` du framework.
...
4. **Gardes de route par domaine** : `guard_prefix` enveloppe, en une passe
   ...
   du même domaine.
   Socle → `socle.gerer`, conception → `conception.gerer`,
...
conception/exécution/suivi.
L'élève viendra avec une UI dédiée et un accès
...
- **Positif** : nav et routes réellement pilotées par le rôle, vérifiées de bout
  ...
  anonyme → 403).
  Aucune dépendance à une API dépréciée.
  Le contrat reste la
  seule source de vérité, versionnable avec le code.
  `make check` vert.
- **Négatif / dette** : on **contourne** le resolveur de l'opt-in au lieu de
  ...
  l'auth moderne rendrait la couche `can()`/gardes redondante).
  Le `can()` maison
  s'appuie sur l'**ordre d'enregistrement** des providers (le nôtre en dernier) :
  contrat implicite à surveiller.
  Les sous-entités hors nav sont gardées par leur
...
`request.roles`.
Le RBAC contractuel natif est donc **câblable** sur l'auth moderne.
...
- **`current_user_roles` → middleware.** Le natif attend que l'app **injecte**
  `request.roles`.
  Il faut donc un middleware qui, par requête, résout les slugs de
  rôles de l'utilisateur connecté (auth moderne → `user_roles`/`roles`) et les pose
  sur la requête.
  C'est le même travail que `current_user_roles`, déplacé en amont.
- **`guard_prefix` sans équivalent natif** : le RBAC natif protège route par route
  (décorateur sur handler), sans passe par préfixe d'URL.
  À **conserver** en maison.
- **Modèle base vs contrat** : le `can()` natif base exige les tables
  `permissions`/`role_permissions` (absentes).
  On reste sur le **contrat** natif
...
- **Remplir aussi la session dépréciée** au login (dict user + `roles`) pour que
  ...
  **API dépréciées** (avertissements, cassables en Forge 2.0).
  Écarté au profit
  de la robustesse.
- **Modèle base (tables `permissions`/`role_permissions`) + resolveur DB natif** :
  ...
  contrat et la base (pas de `rbac:sync`).
  Deux sources de vérité pour aucun gain
  ici.
  Écarté.
- **Attendre un correctif Forge** : bloquerait la discrimination par rôle, alors
  que le besoin est immédiat et le contournement isolé et réversible.
  Écarté.

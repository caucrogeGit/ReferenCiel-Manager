...
encore de backend BDD** (ADR-005 proposé) ni d'entité. Il faut donc un socle de
...
runtime reste 100% Forge, ADR-004).
...
- Marqueurs :
  ...
  - **`db`** — intégration nécessitant un backend BDD installé ; **sauté** tant
    qu'aucun backend n'est présent (repris dès l'ADR-005 appliqué).
...
- **Réinventer un harnais maison** au lieu de `forge-mvc-testing` : rejeté —
  contraire au « 100% Forge » et au rôle de banc d'essai (ADR-004/005).

...
referentiel_niveau_classe`) sont le **socle métier** de l'application. Ils doivent :
...
`make:command`, pas de `mvc/commands/`). Un chargement scripté est donc la voie
...
1. **Dossier livré** : `data/referentiels/` accueille les JSON canoniques de
   référentiel **versionnés**. C'est la **source unique** — le jeu de démo lui-même
...
- **Réimport impossible à chaud** si des données applicatives (ex. `evaluation_critere`)
  ...
  externe l'en empêche. Sans objet pour l'usage visé (chargement à l'installation, base

...
applicative réelle et renvoie la **MFA** (temps T2) à un ADR dédié.
Le cadrage
l'exige : « les comptes professeur et administrateur **doivent pouvoir utiliser la
MFA** » (instructions §20, cadre §10).

L'opt-in officiel **`forge-mvc-mfa`** 1.0.0rc2 est installé.
...
ni accès base.
Comme pour le RBAC ([ADR-013](013-rbac-couche-fine-maison-sur-contrat.md))
...
1. **MFA optionnelle, en self-service.** Chaque utilisateur connecté active (ou
   désactive) sa MFA depuis une page **« Sécurité »** (`/securite`).
   Cela satisfait
   le « doivent pouvoir » du cadrage sans imposer de friction.
   Le login ne déclenche
...
2. **Facteurs : TOTP + codes de secours.** Enrôlement TOTP (QR / URI `otpauth://`
   ...
   fois** à l'activation.
   Le challenge de login accepte un code TOTP **ou** un code
...
3. **Réutiliser l'opt-in, ne rien réimplémenter.** Enrôlement via
   ...
   L'app ne fait que la **persistance** (SQL visible et
   paramétré) et le câblage HTTP.

4. **Secret TOTP chiffré au repos.** Le secret n'est **jamais** stocké en clair :
   l'opt-in le chiffre (Fernet, préfixe `enc:`).
   La clé vit dans la variable d'env
   **`FORGE_MFA_SECRET_KEY`** (hors dépôt : `env/dev` ignoré par git, `env/prod` en
   production ; `env/example` documente la génération).
   La validité de la clé est
...
5. **Challenge au login (anti-fixation préservée).** Après le mot de passe validé :
   ...
   vers `/login/mfa`.
   À la vérification réussie : `login_user` **puis**
   `regenerate_session` (même défense anti-fixation que le login simple).
   Le
...
7. **Step-up sur la désactivation.** Retirer la MFA est une action sensible : elle
   ...
   On utilise l'**API native** de l'opt-in
   ...
   > / F42) et on la contournait par une vérification directe du facteur.
   > **F42 corrigé
...
- **Positif** : capacité MFA réelle (exigence cadrage §20), robuste (codes de
  ...
  réversible.
  Les tables MFA sont enfin migrées.
- **Négatif / limites** : l'anti-rejeu de l'opt-in est **in-memory process-local**
  (non partagé entre workers), limitation assumée, fenêtre de risque < 30 s.
  La
...
- **MFA obligatoire pour prof/admin.** Rejetée pour ce premier incrément : impose un
  ...
  « doivent pouvoir » du cadrage.
  Réexaminable si une politique l'exige.
- **TOTP seul, sans codes de secours.** Rejetée : un authenticator perdu bloquerait
  le compte (récupération seulement par un admin).
  Le coût des codes de secours est

...
1. **Résolution des rôles = native.** On s'appuie sur `get_request_roles`
   ...
   **publique** qui affiche du RBAC, la home connectée).
   Plus de `current_user_roles`.
...
- **Positif** : dette F30 close, **zéro code RBAC maison**, une seule source de
  vérité (contrat) et un seul resolveur (natif).
  Résolution des rôles centralisée.
...
- **Retrait partiel** (version initiale de cet ADR, avant les correctifs A/B/C) :
  ...
  d'équivalents natifs.
  Rendu **caduc** par les correctifs Forge : le retrait total

...
`forge new`).
RéférenCiel Manager, en tant que **banc d'essai** (ADR-006), doit
...
déplaçable**.
Elle a aussi imposé un **force-push** qui réécrit l'historique public.
...
1. **Montée en place, jamais par déplacement.** On récupère les apports du
   squelette **dans le dossier existant** du projet.
   Aucun dossier n'est déplacé ni
   rencommé : le `.venv` et le `.git` (historique + `origin`) restent en place.
   Le push est un **fast-forward** (pas de réécriture d'historique).
...
2. **Manifeste de propriété.** L'opération est pilotée par un manifeste explicite
   ...
   **fusion idempotente**.
   Le manifeste et la procédure rejouable vivent dans
...
3. **Environnement rafraîchi sur place.** Le `.venv` est mis à jour par
   `make setup`.
   Piège documenté : un pin git à **version inchangée** (`1.0.0rc2`)

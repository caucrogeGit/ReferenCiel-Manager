...
def _form_data_from_tentative_qcm(record: dict) -> dict:
    """Convertit les colonnes SQL vers les noms de champs du formulaire."""
    return {
        "numero_tentative": record.get("NumeroTentative"),
...
class TentativeQCMController(BaseController):
    ...
    def _list_context(request):
        ...
        tentative_qcms = find_tentative_qcms_paginated(
    ...
    def create(request: Request) -> Response:
        ...
        if not form.is_valid():
            return BaseController.validation_error("app/tentative_qcm/form.html",
                ...
                request=request)
        add_tentative_qcm(form.cleaned_data)
        return BaseController.redirect_with_flash(request, "/tentative_qcm", "TentativeQCM créé.")

    @staticmethod
    def show(request: Request) -> Response:
        id = TentativeQCMController._parse_id(request.route("id"))
        if id is None:
    ...
    def update(request: Request) -> Response:
        ...
        if not form.is_valid():
            return BaseController.validation_error("app/tentative_qcm/form.html",
                ...
                request=request)
        update_tentative_qcm(id, form.cleaned_data)
        return BaseController.redirect_with_flash(
            request, f"/tentative_qcm/show/{id}", "TentativeQCM mis à jour.")
    ...
    def bulk_delete_confirm(request: Request) -> Response:
        ...
        if not ids:
            return BaseController.redirect_with_flash(request, "/tentative_qcm", "Aucun élément sélectionné.")
        bulk_delete_tentative_qcms(ids)
        count = len(ids)
        return BaseController.redirect_with_flash(
            request, "/tentative_qcm",
    ...
    def export_csv(request: Request) -> Response:
        ...
        for row in rows:
            writer.writerow([TentativeQCMController._csv_escape(str(row.get(key) or "")) for _, key in _CSV_COLS])
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": 'attachment; filename="tentative_qcms.csv"',

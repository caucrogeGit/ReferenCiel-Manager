...
def lignes_evaluation(progression_eleve_id: int) -> list[dict[str, Any]]:
    ...
    return fetch_all(
        ...
        (progression_eleve_id,),
    )


def agreger_synthese(progression_eleve_id: int) -> list[dict[str, Any]]:
    ...
    for ligne in lignes_evaluation(progression_eleve_id):
...
def creer_bilan(
    *, progression_eleve_id: int, professeur_id: int, appreciation: str, statut: str
) -> int | None:
    ...
    """
    prog = fetch_one(
        "SELECT eleve_id FROM progression_eleve WHERE Id = ?", (progression_eleve_id,)
    )
    if prog is None:
        return None
    eleve_id = int(prog["eleve_id"])
    synthese = json.dumps(agreger_synthese(progression_eleve_id), ensure_ascii=False)
    return insert(

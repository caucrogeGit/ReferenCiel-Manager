...
UPDATE       = "UPDATE affectation_professeur_classe SET Role = ?, professeur_id = ?, classe_id = ?, annee_scolaire_id = ?, UpdatedAt = ? WHERE Id = ?"
...
def add_affectation_professeur_classe(data):
    return insert(INSERT, (data["role"], data["professeur_id"], data["classe_id"], data["annee_scolaire_id"], datetime.now(timezone.utc), datetime.now(timezone.utc),))


def update_affectation_professeur_classe(id, data):
    execute(UPDATE, (data["role"], data["professeur_id"], data["classe_id"], data["annee_scolaire_id"], datetime.now(timezone.utc), id))

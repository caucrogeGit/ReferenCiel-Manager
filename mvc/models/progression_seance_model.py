...
SELECT_ALL   = "SELECT progression_seance.*, progression_sequence.Statut AS progression_sequence_id_label, seance.Titre AS seance_id_label FROM progression_seance LEFT JOIN progression_sequence ON progression_seance.progression_sequence_id = progression_sequence.Id LEFT JOIN seance ON progression_seance.seance_id = seance.Id ORDER BY progression_seance.Id"
SELECT_BY_ID = "SELECT progression_seance.*, progression_sequence.Statut AS progression_sequence_id_label, seance.Titre AS seance_id_label FROM progression_seance LEFT JOIN progression_sequence ON progression_seance.progression_sequence_id = progression_sequence.Id LEFT JOIN seance ON progression_seance.seance_id = seance.Id WHERE progression_seance.Id = ?"
INSERT       = "INSERT INTO progression_seance (Statut, progression_sequence_id, seance_id, CreatedAt, UpdatedAt) VALUES (?, ?, ?, ?, ?)"
UPDATE       = "UPDATE progression_seance SET Statut = ?, progression_sequence_id = ?, seance_id = ?, UpdatedAt = ? WHERE Id = ?"
...
def add_progression_seance(data):
    return insert(INSERT, (data["statut"], data["progression_sequence_id"], data["seance_id"], datetime.now(timezone.utc), datetime.now(timezone.utc),))


def update_progression_seance(id, data):
    execute(UPDATE, (data["statut"], data["progression_sequence_id"], data["seance_id"], datetime.now(timezone.utc), id))
...
_ALLOWED_SORT = {"statut": "progression_seance.Statut", "progression_sequence_id": "progression_seance.progression_sequence_id", "seance_id": "progression_seance.seance_id", "created_at": "progression_seance.CreatedAt", "updated_at": "progression_seance.UpdatedAt", "id": "progression_seance.Id"}
_ALLOWED_FILTERS = {"progression_sequence_id": "progression_seance.progression_sequence_id", "seance_id": "progression_seance.seance_id"}
...
def find_progression_seances_paginated(q: str | None = None, sort: str | None = None, direction: str = "asc", limit: int = 10, offset: int = 0, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    base = "SELECT progression_seance.*, progression_sequence.Statut AS progression_sequence_id_label, seance.Titre AS seance_id_label FROM progression_seance LEFT JOIN progression_sequence ON progression_seance.progression_sequence_id = progression_sequence.Id LEFT JOIN seance ON progression_seance.seance_id = seance.Id"
    clauses: list[str] = []
    params: list[Any] = []
    if q and _SEARCH_COLS:
        clauses.append("(" + " OR ".join(c + " LIKE ?" for c in _SEARCH_COLS) + ")")
...
def get_progression_sequence_choices():
    rows = fetch_all("SELECT Id, Statut FROM progression_sequence ORDER BY Statut")

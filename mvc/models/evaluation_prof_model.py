...
professeur : **valider une séance** (`progression_seance.Statut`) et **confirmer
...
def get_progression_detail(progression_id: int) -> dict[str, Any] | None:
    ...
    entete["seances"] = fetch_all(
        "SELECT pp.Id AS progression_seance_id, pa.Ordre AS ordre, pa.Titre AS titre, pp.Statut AS statut, "
        "(SELECT MAX(Score) FROM tentative_qcm t WHERE t.progression_seance_id = pp.Id) AS meilleur_score, "
        "(SELECT COUNT(*) FROM item_coche ic WHERE ic.progression_seance_id = pp.Id AND ic.CocheEleve = 1) AS coches_eleve, "
...
def set_seance_statut(progression_seance_id: int, statut: str) -> bool:
    """Pose le statut d'une séance (valeur contrôlée). False si le statut est invalide."""
    if statut not in STATUTS_SEANCE:
        return False
    execute(
        "UPDATE progression_seance SET Statut = ? WHERE Id = ?",
        (statut, progression_seance_id),
    )
    return True
...
def get_checklist_review(progression_seance_id: int) -> dict[str, Any] | None:
    ...
    for section in sections:
        section["items"] = fetch_all(
            ...
            "LEFT JOIN item_coche ic ON ic.item_id = i.Id AND ic.progression_seance_id = ? "
...
def enregistrer_coches_prof(
    progression_seance_id: int, items_coches: set[int]
) -> dict[str, Any] | None:
    ...
    seance = fetch_one(
        "SELECT seance_id AS seance_id FROM progression_seance WHERE Id = ?", (progression_seance_id,)
    )
    if seance is None:
        return None
    checklist = fetch_one(

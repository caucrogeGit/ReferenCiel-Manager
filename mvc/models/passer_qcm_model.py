...
def _qcm_du_palier(palier_id: int) -> dict[str, Any] | None:
    return fetch_one(
        "SELECT Id AS id FROM qcm WHERE palier_id = ? ORDER BY Id LIMIT 1", (palier_id,)
    )
...
def get_qcm_a_passer(progression_palier_id: int, user_id: int) -> dict[str, Any] | None:
    ...
    if palier is None:
        return None
    qcm = _qcm_du_palier(int(palier["palier_id"]))
    if qcm is None:
        return None
    questions = fetch_all(

...
UPDATE       = "UPDATE choix_qcm SET Lettre = ?, Texte = ?, question_id = ?, UpdatedAt = ? WHERE Id = ?"
...
def add_choix_qcm(data):
    return insert(INSERT, (data["lettre"], data["texte"], data["question_id"], datetime.now(timezone.utc), datetime.now(timezone.utc),))


def update_choix_qcm(id, data):
    execute(UPDATE, (data["lettre"], data["texte"], data["question_id"], datetime.now(timezone.utc), id))

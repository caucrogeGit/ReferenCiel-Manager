...
Le professeur note l'activité d'un palier au moyen des **critères observables**
...
(progression_palier, activité) ; chaque `evaluation_critere` fige le niveau d'un
...
def _contexte(progression_palier_id: int) -> dict[str, Any] | None:
    """Résout le palier : activité à noter, élève, et professeur de l'affectation."""
    return fetch_one(
        "SELECT pp.Id AS pp_id, pp.progression_eleve_id AS progression_id, "
...
def _evaluation_existante(progression_palier_id: int, activite_id: int) -> int | None:
    row = fetch_one(
        ...
        "WHERE progression_palier_id = ? AND activite_id = ? ORDER BY Id LIMIT 1",
        (progression_palier_id, activite_id),
    )
    return int(row["id"]) if row is not None else None


def get_grille(progression_palier_id: int) -> dict[str, Any] | None:
    """Grille de notation : compétences → critères (+ niveau déjà posé), si le
    palier a une activité à évaluer. None sinon."""
    ctx = _contexte(progression_palier_id)
    if ctx is None or ctx["activite_id"] is None:
        return None
    eval_id = _evaluation_existante(progression_palier_id, int(ctx["activite_id"])) or 0
...
def enregistrer_notation(
    ...
    with transaction() as tx:
        eval_id = _evaluation_existante(progression_palier_id, activite_id)
        if eval_id is None:
            eval_id = insert(

...
UPDATE       = "UPDATE section_checklist SET Numero = ?, Titre = ?, checklist_id = ?, UpdatedAt = ? WHERE Id = ?"
...
def add_section_checklist(data):
    return insert(INSERT, (data["numero"], data["titre"], data["checklist_id"], datetime.now(timezone.utc), datetime.now(timezone.utc),))


def update_section_checklist(id, data):
    execute(UPDATE, (data["numero"], data["titre"], data["checklist_id"], datetime.now(timezone.utc), id))

...
vs bonne réponse), la mise à jour du **statut** du palier (100 % → validé, sinon en
cours), et le **contrôle d'appartenance** (palier d'autrui → None). CI-safe (ADR-006).
"""
...
def test_tout_juste_valide_le_palier(monkeypatch: pytest.MonkeyPatch) -> None:
    cap = _install(
        monkeypatch, palier=_PALIER,
    ...
    # garde anti-régression : ne met à jour que si le palier n'est pas déjà validé
    assert "Statut <> ?" in maj[0]


def test_palier_d_autrui_renvoie_none(monkeypatch: pytest.MonkeyPatch) -> None:
    cap = _install(monkeypatch, palier=None, lettres_choix={}, bonnes={})
